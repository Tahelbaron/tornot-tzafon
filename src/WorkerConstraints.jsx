...
export default function WorkerConstraints() {
  ...
  const [militaryNote, setMilitaryNote] = useState("");
  ...
  const submitConstraint = async () => {
    ...
    if (type === "military" && !militaryNote.trim()) return;
    ...
    for (const d of selectedDays) {
      await api.saveConstraint({
        ...
        type,
        day: d,
        shiftLabel: type === "military" ? militaryNote.trim() : null,
      });
    }
    setStatus("✅ נשמר!");
  ...
  const canSubmit = !isPastDeadline && selectedWorker && selectedDays.size > 0 &&
    (type !== "military" || militaryNote.trim());
      ...
      <div style={{background:"linear-gradient(180deg,#111827,#0D1526)",borderBottom:"1px solid #1F2937",padding:"16px 20px"}}>
        ...
        <div style={{
          ...
          {isPastDeadline ? (
            ...
            <>⚠️ נותרו {daysUntilDeadline} ימים להגשה — דדליין: {DEADLINE.toLocaleDateString("he-IL")}</>
          ) : (
              ...
              <>
                {isPastDeadline ? (
                  <div style={{background:"#450a0a",border:"1px solid #EF4444",borderRadius:12,padding:20,textAlign:"center"}}>
                    ...
                    <div style={{fontSize:12,color:"#94A3B8",marginTop:4}}>לא ניתן להוסיף אילוצים חדשים</div>
                  </div>
                ) : (
                  <>
                    {/* סוג אילוץ */}
                    <div style={{background:"#1E293B",border:"1px solid #334155",borderRadius:16,padding:18,marginBottom:14}}>
                        ...
                        {[
                          ...
                          <label key={val} style={{
                            ...
                            <input type="radio" name="type" value={val}
                              checked={type===val}
                              onChange={()=>{setType(val);setSelectedDays(new Set());setMilitaryNote("");}}
                      ...
                      {type==="military" && (
                        <input
                          value={militaryNote}
                          onChange={e=>setMilitaryNote(e.target.value)}
                          placeholder="פרט את האילוץ הצבאי..."
                          style={{
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:3}}>
                        {(() => {
                          ...
                          if (adjustedFirstDay !== null) {
                            for (let i=0; i<adjustedFirstDay; i++) cells.push(<div key={`e${i}`}/>);
                          }
                ...
                {/* האילוצים שלי — תמיד מוצג */}
                {myConstraints.length > 0 && (
                  <div style={{background:"#1E293B",border:"1px solid #334155",borderRadius:16,padding:18,marginTop:8}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:6}}>
                      {myConstraints.sort((a,b)=>(a.day||0)-(b.day||0)).map(c=>{
                        ...
                        const label = c.type==="military" && c.shiftLabel
                          ? `${dayStr} — ${c.shiftLabel}`
                          : dayStr;
                        return (
                          <div key={c.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#0F172A",borderRadius:8,padding:"9px 12px"}}>
                  ...
                  <div key={w.id} style={{background:"#1E293B",border:"1px solid #334155",borderRadius:14,padding:16,marginBottom:10}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:6}}>
                      {wcs.sort((a,b)=>(a.day||0)-(b.day||0)).map(c=>{
                        ...
                        const dow = new Date(viewYear,viewMonth,c.day).getDay();
                        const dayStr = `${DAYS_HE[dow]} ${c.day}.${viewMonth+1}`;
                        const label = c.type==="military"&&c.shiftLabel ? `${dayStr} — ${c.shiftLabel}` : dayStr;
                        return (
