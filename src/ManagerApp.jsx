...
function saveSchedule(year,month,schedule,counts){
  try{localStorage.setItem(`schedule_${year}_${month}`,JSON.stringify({schedule,counts,savedAt:new Date().toISOString()}));}catch(e){}
}
function loadSchedule(year,month){
  try{const r=localStorage.getItem(`schedule_${year}_${month}`);return r?JSON.parse(r):null;}catch{return null;}
}
function clearSchedule(year,month){
  try{localStorage.removeItem(`schedule_${year}_${month}`);}catch{}
}
function saveWorkers(workers){
  try{localStorage.setItem("workers",JSON.stringify(workers));}catch{}
}
function loadWorkers(){
  try{const r=localStorage.getItem("workers");return r?JSON.parse(r):null;}catch{return null;}
}
...
// תורנויות שדורשות יום הכנה לפני (לא נותנים לעובד שיש לו אילוץ יום לפני)
const NEEDS_PREP_DAY = ["a1","a2","a3","a4","a5","a6","a7","a8","a9","a10"];
// a11=עתודה 1, a12=עתודה 2, a2=פיצול — אלו לא דורשות הכנה
// רק: מעצרים,משלב,הרכב,דן יחיד,תזכורות,תעבורה

function getPrevWorkday(year,month,day){
  // יום קודם — אם ראשון (0) אז יום חמישי (day-3), אחרת day-1
  const dow=new Date(year,month,day).getDay();
  if(dow===0) return day-3; // ראשון → חמישי
  return day-1;
}
    ...
    for(const shift of sorted){
      ...
      const eligible=workers.filter(w=>{
        ...
        for(const c of cs){
          ...
          // חוק יום לפני — רק לתורנויות שדורשות הכנה
          if(NEEDS_PREP_DAY.includes(shift.id)&&prevDay>=1){
            if((c.type==="unavailable"||c.type==="military"||c.type==="personal")&&c.day===prevDay) return false;
          }
          if(c.type==="shift_off"&&c.shiftLabel===shift.label) return false;
        }
        return true;
      }).sort((a,b)=>{
        const tA=seniorityLoadTarget[a.seniority]||1;
        const tB=seniorityLoadTarget[b.seniority]||1;
        const lrA=counts[a.id].loadTotal/(MAX_MONTH[a.seniority]*tA);
        const lrB=counts[b.id].loadTotal/(MAX_MONTH[b.seniority]*tB);
        if(a.seniority===b.seniority){
          ...
          return(0.5*lrA+0.3*dvA+rA)-(0.5*lrB+0.3*dvB+rB);
        }
      ...
      // fallback — אם אין מועמדים, שים מישהו גם על אילוץ (וסמן)
      let chosen=eligible[0];
      let isOnConstraint=false;
      if(!chosen){
        const fb=workers.filter(w=>{
          return todayCount[w.id]<(MAX_DAY[w.seniority]||2)&&
        ...
        chosen=fb[0];
        isOnConstraint=!!chosen;
      }

      if(chosen){
        schedule[d][shift.id]=chosen.id;
        if(isOnConstraint) schedule[d][`${shift.id}_conflict`]=true;
...
const Btn=({children,onClick,variant="primary",small,disabled,style={}})=>(
  <button onClick={onClick} disabled={disabled} style={{
    ...
    background:variant==="primary"?"linear-gradient(135deg,#3B82F6,#6366F1)":variant==="danger"?"#450a0a":"transparent",
    color:variant==="ghost"?"#94A3B8":"#fff",
    border:variant==="ghost"?"1px solid #334155":variant==="danger"?"1px solid #EF4444":"none",
...
const Inp=({value,onChange,placeholder,style={}})=>(
  <input value={value} onChange={onChange} placeholder={placeholder} style={{padding:"8px 12px",borderRadius:8,border:"1px solid #334155",background:"#0F172A",color:"#E2E8F0",fontFamily:"'Heebo',sans-serif",fontSize:13,outline:"none",...style}}/>
);
...
export default function ManagerApp(){
  ...
  const clearAndRegenerate=()=>{
    if(window.confirm("למחוק את התורנות הנוכחית וליצור חדשה?"))
      {clearSchedule(year,month);generate();}
  };
  ...
  const addWorker=()=>{
    ...
    const updated=[...workers,{id:maxId+1,name:newWorkerName.trim(),seniority:newWorkerSeniority}];
    setWorkers(updated);setNewWorkerName("");setSchedule(null);
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:"#080F1A",fontFamily:"'Heebo',sans-serif",direction:"rtl",color:"#E2E8F0"}}>
      ...
      <div style={{background:"linear-gradient(180deg,#111827,#0D1526)",borderBottom:"1px solid #1F2937",padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        ...
        <TabBar tabs={[["schedule","לוח"],["load","עומס"],["workers","עובדים"]]} active={tab} onChange={setTab}/>
      </div>
          ...
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                ...
                {conflictCount>0&&(
                  <span style={{fontSize:11,color:"#F59E0B",background:"#F59E0B22",borderRadius:6,padding:"3px 8px",fontWeight:700}}>
                    ⚠️ {conflictCount} שיבוצים על אילוץ
                  </span>
                )}
            ...
            {savedAt&&(
              <div style={{fontSize:11,color:"#475569",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
                ...
                <button onClick={()=>{clearSchedule(year,month);setSchedule(null);setCounts({});setSavedAt(null);}}
                  style={{background:"none",border:"none",cursor:"pointer",color:"#EF4444",fontSize:11,padding:0}}>(מחק)</button>
              </div>
            )}
              ...
              <div style={{background:"#111827",borderRadius:18,border:"1px solid #1F2937",overflow:"hidden"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)"}}>
                  ...
                  {Array.from({length:daysInMonth},(_,i)=>i+1).map(day=>{
                    ...
                    // בדוק אם יש שיבוץ על אילוץ היום
                    ...
                    return(
                      ...
                      >
                        <div style={{fontWeight:800,fontSize:12,color:isToday?"#6366F1":hasPink?"#C084FC":"#374151",marginBottom:2,display:"flex",alignItems:"center",gap:3}}>
                          {day}
                          {hasConflict&&<span style={{fontSize:8,color:"#F59E0B"}}>⚠️</span>}
                          {hasPink&&<span style={{fontSize:8,color:"#E49EDD"}}>●</span>}
                        </div>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:2}}>
                          {workerIds.slice(0,4).map(wid=>{
                            ...
                            const wHasConflict=wShifts.some(s=>schedule[day]?.[`${s.id}_conflict`]);
                            return(
                              <div key={wid} style={{
                                background:wHasConflict?"#F59E0B33":senColor(w.seniority)+"22",
                                color:wHasConflict?"#F59E0B":senColor(w.seniority),
                                borderRadius:3,padding:"1px 4px",fontSize:9,fontWeight:700}}>
                                {w.name}{wHasConflict?" ⚠️":""} <span style={{opacity:0.6}}>({load})</span>
                              </div>
                            );
                          })}
              ...
              return(
                <div key={sen} style={{marginBottom:20}}>
                  <div style={{fontSize:12,fontWeight:700,color:senColor(sen),marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
                    ...
                    {senLabel(sen)} · {group.length} עובדים · עד {MAX_MONTH[sen]}/חודש
                  </div>
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#111827",border:"1px solid #1F2937",borderRadius:20,padding:24,width:"100%",maxWidth:560,maxHeight:"88vh",overflowY:"auto",direction:"rtl"}}>
            ...
            {dayActiveShifts(dayModal).some(s=>schedule[dayModal]?.[`${s.id}_conflict`])&&(
              <div style={{background:"#F59E0B22",border:"1px solid #F59E0B",borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,color:"#F59E0B",fontWeight:700}}>
                ⚠️ יש ביום זה שיבוצים על אילוץ — נדרשת בדיקה ידנית
              </div>
            )}
                    ...
                    {shShifts.map(shift=>{
                      ...
                      return(
                        <div key={shift.id} style={{background:"#0F172A",borderRadius:10,padding:"9px 12px",
                          border:`1px solid ${isConflict?"#F59E0B":worker?shift.color+"40":"#450a0a55"}`,
                        ...
                        {shifts.map(s=>{
                          ...
                          return(
                            <span key={s.id} style={{background:isConflict?"#F59E0B22":s.bg,color:isConflict?"#F59E0B":s.dark,borderRadius:6,padding:"3px 10px",fontSize:12,fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
                              {s.label}
                              {isConflict&&<span style={{fontSize:9}}>⚠️</span>}
